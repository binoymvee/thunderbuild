...
def verifyIPConnection(ipAddress, networkInfo, cloudId, networkType, netmask, vlanTag = False):
    '''
    ...
    if not ipAddress:
        msgList[" -- No valid ip address found in ip network range"] = 0
    ...
    if nodeList:
        ...
        # check ping of the ip address from thunder
        if not checkPing(ipAddress):
            statusMsgList.append("Failed")
            msgList[" -- Connection to nodes failed in network!"] = 0
    else:
...
def verifyPublicNetwork(cloudId, networkInfo):
    ...
    if netList:
        ...
        for netIPInfo in netList:
            ...
            [retMsg, msgList] = verifyIPConnection(ipAddress, networkInfo, cloudId, netType, netmask, vlanTag)
...
def saveNodeDiskInfo(nodeInfo, diskContent):
    ...
    if diskList:
        ...
        for info in diskList:
            ...
            if len(diskInfo) >= 2:
                diskName = diskInfo[0]
                systemSpace = settings.SYSTEM_PARTITON_SPACE if diskName == "/dev/sda" else 0
...
def getNetworkVlanTag(networkInfo, networkType):
    '''
                ...
                if len(networkList) > 1:
                    ...
                    for netType in networkList:
                        ...
                        if netType == "A":
                            ...
                            if getNetworkVlanTag(networkInfo, netType):
                                ...
                                if netType == "S" and not networkInfo.st_network_cidr:
                                    ...
                                    return False, "More than one network configured on single NIC"
