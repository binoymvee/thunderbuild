        ...
        for nicName, netType, nicIp in list:
            ...
            if netType == 'P':
                ...
                for cidrVal, tagVal in vlanTag.iteritems():
                    ...
                    # if public network cidr
                    if cidrVal == networkInfo.public_cidr:
                        networkList[netType]['vlan_tag'] = vlanTag[cidrVal] if vlanTag[cidrVal] else ''
                    else:
                ...
                extBridgeInterface = nicName
                
                # if multiple external network, then remove external network bridge, bridge mappings
                if floatNetList:
                    ...
                    assignSpecAttribute(specAttrList, specList['override_attributes'], "physnet1:1:2999")
                    specAttrList = ['openstack', 'network', 'openvswitch', 'bridge_mappings']
                    assignSpecAttribute(specAttrList, specList['override_attributes'], "")
                elif networkList[netType]['vlan_tag']:
                    extBridgeInterface = nicName + "." + str(networkList[netType]['vlan_tag'])
                
                # assign external interface
                specAttrList = ['openstack', 'network', 'l3', 'external_network_bridge_interface']
                assignSpecAttribute(specAttrList, specList['override_attributes'], extBridgeInterface)
...
def updateDeploymentEnvironment(cloudId, nodeInfo, roleCode = "", overrideList = {}):
    ...
    specList['override_attributes']['thunder']['os_installation_disk'] = settings.OS_INSTALLATION_DISK
...
def updateCloudNetworkEnvironment(cloudId, specList):
    ...
    for floatingIP in floatingIPList:
        ...
        ipInfo = {
            ...
            "ip_cidr": cidrVal,
            "vlan_tag": str(floatingIP.vlan_tag) if floatingIP.use_vlan else "",
        }
