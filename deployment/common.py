...
def installOpenstackOS(nodeInfo, envName):
    ...
    if not nodeInfo.currentos:
        ...
        try:
            cobblerCmd = "cobbler system add --name=" + nodeInfo.host_name + " --profile=" + settings.COBBLER_UBUNTU_PROFILE + " --ip-address=" + nodeInfo.node_ip
...
def deployRoleToNode(nodeInfo, recipePrList, jobId, roleCode):
    ...
    if jobStatus == 'S':
        ...
        if jobStatus == 'S':
            ...
            for priority, recipeList in recipePrList.iteritems():
                ...
                # if object storage related deployment
                if ('openstack-object-storage::setup' in recipeNameList) or ('role[os-object-storage-account]' in recipeNameList):
    ...
    if isCloudDeploymentComplete(nodeInfo.cloud_id):
        ...
        if len(jobList):
            ...
            dashboardLink = getCloudDashBoardLink(nodeInfo.cloud_id)
            message = "'" + cloudnfo.cloud_name + "' deployed successfully in " + dashboardLink
            params = {
                'alert_type': 'Cloud', 
                'referece_id': nodeInfo.cloud_id,
...
def deployRevokeNodeRole(nodeInfo, jobId, roleCode):
    ...
    if isCloudDeploymentComplete(nodeInfo.cloud_id):
        ...
        if len(jobList) == 0:
            deleteCloud(nodeInfo.cloud_id)
    
    return returnInfo


def deleteCloud(cloudId):
    '''
...
def getCloudDashBoardLink(cloudId):
    """
    ...
    if controllerIPList:
        dashboardLink = "http://" + controllerIPList[0]['ip'] + "/"
