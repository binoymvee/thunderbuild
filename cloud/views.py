        ...
        if haveTag == False:
            ...
            for mapData in mapDatas:
                ...
                if mapData.network_type != mappingDetail.network_type:
                    hasTag = getNetworkVlanTag(networkInfo, mappingDetail.network_type)
                    if hasTag == False:
                        networkError = True
...
def logTab(request):
    ...
    # Get the log details
    cursor = connection.cursor()
    sql = """SELECT log.log_type, log.log_title, log.log_details FROM thunder_nodelog as log
            INNER JOIN thunder_nodelist as list ON list.id = log.node_listid
            INNER JOIN thunder_noderole as role ON list.id= role.node_id"""
    cursor.execute(sql);
...
def removeRole(request):
    ...
    for  ids in eval(request.POST.get('datepost')):
        ...
        for nodeRole in nodeRoles:
            ...
            job = Job.objects.filter(subject_id = nodeRole.id, job_type = 'ROLE_ASSIGN')
...
#                 #If all the nodes are removed then resetting the node list
#                 if len(nodeRoles) == 0:
